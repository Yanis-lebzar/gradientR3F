...
let stripePalette = ["#FF323E", "#88D2F1", "#FFC957", "#AA56EE"];
stripePalette = stripePalette.map((color) => new THREE.Color(color));
console.log(stripePalette, "stripe");

export const ShaderGradient = shaderMaterial(
  {
    uTime: 0,
    uColor: stripePalette,
  },
  glsl`
  uniform float uTime;
uniform vec2 pixels;
uniform vec3 uColor[4];
  ...
  void main() {
vec2 noiseCoord = uv * vec2(9.,10.);
float tilt = -0.2 *uv.y;
float incline = uv.x*0.2;
float offset = incline * mix(-0.26,0.26,uv.y);
float noise = snoise(vec3(noiseCoord.x + uTime * 30.,noiseCoord.y, uTime * 10.));
noise = max(0., noise);
...
vUv = uv;
  ...
  `,
  glsl`#pragma glslify: cnoise3 = require(glsl-noise/classic/3d.glsl) 
